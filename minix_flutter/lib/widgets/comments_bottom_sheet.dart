...
Future<void> openCommentsBottomSheet(BuildContext context, int tweetId) async {
  // ✅ 서버용 CommentService 등록 (tweetId별로 tag로 분리)
  final serviceTag = 'comment_service_$tweetId';
  final controllerTag = 'comments_$tweetId';

  // Service (ApiCommentService) 생성/등록
  if (!Get.isRegistered<CommentService>(tag: serviceTag)) {
    final api = Get.find<ApiService>();
    Get.put<CommentService>(
      ApiCommentService(api: api),
      tag: serviceTag,
    );
  }
  ...
  if (!Get.isRegistered<CommentController>(tag: controllerTag)) {
    Get.put(
      CommentController(
        tweetId: tweetId,
        service: Get.find<CommentService>(tag: serviceTag),
      ),
      tag: controllerTag,
    );
  }
  ...
  await showModalBottomSheet(
    ...
    builder: (_) {
      return _CommentsSheet(tweetId: tweetId, tag: controllerTag);
    },
  );

  // ✅ 시트 닫힌 후 정리 (원하면 유지해도 됨)
  if (Get.isRegistered<CommentController>(tag: controllerTag)) {
    Get.delete<CommentController>(tag: controllerTag, force: true);
  }
  if (Get.isRegistered<CommentService>(tag: serviceTag)) {
    Get.delete<CommentService>(tag: serviceTag, force: true);
  }
}
...
class _InputBarState extends State<_InputBar> {
  ...
  @override
  Widget build(BuildContext context) {
    final c = Get.find<CommentController>(tag: widget.tag);
